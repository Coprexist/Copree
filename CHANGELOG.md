...
- **解锁点重写账本**（第四批 b）：`context_sync.rewrite_context` 是唯一允许动中段的地方——
  摘要 + 原样搬运的事件 + 最近 N 条；`executor._unlock_context` 把「重写账本 + 复位便签副本 +
  应用挂起配置/能力变更」收成一处，调用前与工具循环两条路径共用（顺带修掉调用前那条半解锁）。
  补的洞：`history_service.clear` 此前没有调用方，账本只进不出 → 压缩压了等于没压、请求体无上限
...
- **记忆注入移出锁定段**（同批最重要的一处）：`injected_skills` 的检索词是「最近 5 条消息」，
  却被拼进 message 0 → 群里有新消息时**前缀从第 0 字节起全 miss**。群聊/私信两条路径都改成沉到尾部读数
- **记忆索引走版本链冻结**：索引（只看 agent.id，稳定）回锁定段并挂上 `memory-index-{id}` 源（冻结 + 尾部
  差分通知复用能力版本链）；召回 + 技能注入（检索词是当轮消息）留尾部读数。写记忆不再断前缀
...
- **跨对话回复的「读」**：新工具 `read_conversation` 读别的会话最近几条原文（群传 `group_id`、私信传
  ...
  补上读之后「跨对话回复」才成立
- **便签投递语义**：40 次调用有效期内**每个会话各投一份**（判据是该会话自己的账本/帧副本，不是全局标记）；
  整套解锁后若记录仍有效，该会话会**重新拿到**（用户 2026-09-25 确认）
- 验证：全量 **278/0**；新测试走完「投递一次 / 幂等 / 撤下通知 / 解锁清干净 / 解锁后重投」五段；**插一条新消息前后 message 0 字节一致**，
  写一条记忆前后 `message 0` 同样字节一致（版本链 v1→v2、`known=2 / effective=1`，解锁才对齐）；
  首个差异下标 0 → 4（只断在新追加的那条上）；
...
- 阈值**三档**：`T_post`（压后上限 ≈12%）< `T_idle`（久未活跃触发，`T_post + (1/e)(T_hot − T_post)` ≈ 29.7%）< `T_hot`（热触发，主站 DB 60% / 世界 60%）；
