...
- **触发组合规则**：插件、平台内置与 AI 自写的决策技能共用一套「条件组合 + 动作」规则。条件支持
  and/or/not 与自定义运算、判词（自己手搓），动作支持内置（deliver 投递、silent 静默）与自定义注册，
  once 决定投几次（默认按会话投一次，compact 后自动再投）。状态挂状态帧，没规则盯着的工具调用零开销。
  第一个用例：web_search 的「先回复、再核实」不再常驻系统提示，改为首次搜完投一次。
