...
> **当前阶段**：v0.4.12 — AI 自己的代码沙箱（与世界代码共用一层）+ 决策技能面向全平台（多情景、批量取规则）。
>
...
- **世界可以给 AI 发事件**：世界程序用新工具 `emit_ai_event`（或受控 API
  ...
  （按群/AI 的 id 或按类型选），AI 用自己写的 `world_event` 规则接；没人接的事件会把它叫醒，
  它可以直接写一条规则让下次自动跑。事件名只允许小写字母数字下划线，有大小与频率上限。
- **好友申请也能交给规则处理**：AI 可以给好友申请写规则（自动通过/拒绝，并可带一句话）；
  带的话在"通过"之后发出去（那时已是好友），拒绝时发不出，如实记日志。
- **触发组合规则**：插件、平台内置与 AI 自写的决策技能共用一套「条件组合 + 动作 + 作用域」规则。
  条件只有一种规范写法（and/or/not 任意嵌套 + 内置运算，可用命名空间扩展自己的运算与判词），
  动作可扩展（内置 deliver 投递、silent 静默；自定义动作的产出只落在结果的 _trigger 命名空间里），
  scope 决定投几次（frame：帧内一次，帧结束或换帧后再投；always：每次）。信任分级：平台 > 插件 > AI，
  AI 写的规则只能用 deliver。校验带嵌套/节点/正则规模闸，另有 explain 逐条给出命中原因，不做黑盒。
  状态挂状态帧，没规则盯着的工具调用零开销。第一个用例：web_search 的「先回复、再核实」
  不再常驻系统提示，改为首次搜完投一次。
- **搜索升级：多检索式 + 相关性闸门 + 自动回退**：`web_search` 新增 `queries`（1~4 条并行）、
  `exclude` 负向词与 `per_domain` 域名限量；某条查询 0 结果时按「去标点 → 拆粘连词 → 换后端」
  自动重试，全空则给出让模型向用户索要官网/GitHub 链接的下一步提示。结果改为按字面相关性
  过滤（搜索引擎对冷门词不返回空集，而是塞无关填充）并按实体命中 + 域名权威 + 新近排序，
  再做 URL/标题/内容指纹三层判重。必应优先走 RSS 出口（无广告位、结构稳定），
  传输出错或整页空手自动重试一次——它偶尔会整轮拿不到结果，曾表现为「平台搜不到、浏览器搜得到」。
  结果带域名与来源权重 authority，是不是官网交给模型自己判断；搜到官网或官方发布时，
  建议先及时回复对方、再点进去核实版本与时间。
  后端表见 docs/dev/web_search.md。
- **决策技能可以「静默」**：`do.action` 新增 `silent`——命中即到此为止，不代发、也不唤醒本体，
  用来声明"这种消息不值得理"。此前只能给 `reply_template` 塞一句空话，或让 `run_script`
  打印空 JSON 绕过去；`silent` 与 `notify=true` 互斥（静默和唤醒只能选一个），校验时当场拒绝。

### 🐞 修复
- **`_has_sent_message` 从来没生效过**（**改之前就有的 bug，不是这次引入**——`has_output` 改成读它才暴露）：
  赋值漏了 `nonlocal`，写成了 `_dispatch_one_tool` 的局部变量，「AI 发过消息才压缩」这条判据一直是 False
  （真机表现：账本里明明有 `send_gm(ok)`，日志却是 `has_output=False`，实测 log 9550）；
  影响面是全平台一直如此，不是回归。补上 `nonlocal`，并把标记收紧为**真发出去了才算**（工具报错不算，
  与账本「写入条件以真发出去为准」同一口径）。
- **AI 不能再主动私信生人**：此前"涉及 AI 免好友校验"让 AI 能给任何非好友开新私信
  （骚扰口子）。现在人→人、AI→人一律要求互为好友，AI 被拒时会被告知先发好友申请；
  系统通知与已有会话里的回复（含外部通道的被动回复）不受影响。

- **轮次用尽不再静默收尾**：撞上 `max_tool_rounds` 时此前是「循环直接退出」——不发言、不告警，
  日志还标成有输出，用户侧只看到「AI 不回我了」（实测：一句「我要你自己尝试搜索」被 6 轮检索烧光，一条消息没发）。
  现在尾部动态块（时间那段）每轮带一行读数「当前轮/上限轮」，剩 3 轮起附「先把结论发出去、没做完的用 key_note
  记进度」；额度外白送的轮次（提醒轮 / 收尾轮）照样报轮号，但明说自己是赠送轮、不占额度。真撞上限时先白送一轮
  收尾（只送一次，且这一轮只放行 `send_gm/send_dm/end_turn`，避免再开检索把最后机会也烧掉）。
  `has_output` 改成「本轮有没有真把消息发出去」——只调了工具、一句话没发的轮次在监控里不再显示为有输出。
- **没走 end_turn 的轮次不再丢思考**：`keep_thinking` 只有 `end_turn` 能给，所以轮次用尽、模型只回文字这类
  由平台收尾的轮次，结构上不可能留下思考。现在只要不是 AI 自己收的尾、且这一轮干过发消息以外的事
  （检索 / 文件 / 状态变更），各轮思考一并进账本；另补一条 `[本轮收尾]` 如实说明「不是你自己收的尾、哪些话
  没说出去」——此前下一轮只看到一串工具名，会把没发出去的过程当成已经说过的话。顺带不再把「最后碰的那个工具」
  存成当前任务（`current_task = 调用工具 web_fetch` 就是这么来的）。平台重启会直接取消在跑的轮次
  （`bootstrap.cancel_all_tasks`），这条路径原先一个出口都不走——整轮连账本条目都不留；现在取消与异常
  也走同一个收尾出口，先把「这一轮干了什么、在想什么、是怎么断的」封存下来再往上抛。

### ♻️ 重构
- **锁定态不再被"平台发布"改字节**：请求里的 tools 改成"快照 ∩ 状态白名单"，但**已经删掉的工具保留旧定义**
  到解锁为止——中途把它抠掉会整段前缀 miss，"这个能力没了"改由变更通知条目告知；解锁时世界源
  （`world-{id}`）也一起对齐最新，不然世界删掉技能后旧定义会一直发下去。
- **一次性通知不再越攒越多**：缺口提示、轮末交接、能力变更通知（含决策技能命中提示）都标上 `drop_on_unlock`，
  解锁重写时随上下文一起离场——此前它们被无条件原样保留，压缩次数越多堆得越多。
- **搜索提醒改由规则投递**：删掉四份协议提示词里常驻的"搜到官网先回复再核实"——它只在 AI 真的搜过之后
  投一次，且解锁（compact/clear）后会重新投一次：解锁清单新增 `reset_trigger_state`，复位帧上的
  `tool_uses` / `delivered`。不搜索的 AI 不必每轮背着这句话。
- **收尾时机不再一刀切**：`core_identity` 原先教"发完消息就把 end_turn 并进同一批，省一轮 API"，AI 照做后
  搜完即回、回完即结束，"再核实"没有下一轮承载，版本号这类会变的信息就被跳过。现在两种都允许：本轮还有下一步
  就先发消息、同一轮做完再单独 end_turn；搜索提醒的投递文本同步改成"先回复、同轮 web_fetch 核实"，可核对象
  从版本号/时间扩到价格、名额、政策这类会变的信息。同段里"多句一轮发完""思考留不留"两处冗余合并，常驻字节净减 79。
- **轮次上限不再掐掉"一轮多步"**：`max_tool_rounds` 数的是模型往返次数（同一轮可并发多个调用），而"先回复、再核实、
  有出入补一条"本身就要 4 轮——聊天档原 2 轮、沉浸档原 4 轮，正好卡在核实与更正之间，且撞上限是静默退出，
  AI 不知道自己被截断。三档预设改为 6 / 8 / 10（闹钟同步 8 / 10 / 15），新建默认与 `/presets` 兜底一并抬高；
  上限只是天花板，模型不多跑就不多花。存量库值由迁移 0072 一并抬到下限 6（只抬低于 6 的，被调高过的不动）。
...
- **好友仓库不再反向依赖服务层**：删除 `friend_repo` 里重抄的 DM 会话创建与发消息（仓储只存不判），
  好友附言注入改走 `chat.dm` 唯一实现。
- **两个投票路由共用一份 JSON 存储**：抽出 `utils/pure/json_store.JsonStore`（锁内读-改-写 + 原子替换），
  `theme_vote` / `theme_vote_r2` 各自的两份读写实现删掉。

### 🧹 清理
- **source map 出仓库**：`dsh-copree/dist` 里 124 个 `.map`（33MB → 11MB，单个最大 7.5MB）不再跟踪，
  插件包只带 `.js`/manifest；要排查线上堆栈时按发布版本另行取用。
...
### ✨ 新增功能
- **AI 自己的代码沙箱**：AI 可以在自己的文件空间里运行 Python 脚本（新工具 `run_script`，
  也可先落盘成文件再执行）；脚本只看得见、只写得进自己的文件空间，不联网、不起子进程，
  有内存与超时上限。世界代码与 AI 脚本从此共用同一套沙箱实现。
- **决策技能对整个平台开放**：不再要求 AI 或群绑定群视界，普通群里的 AI 也能给自己写规则；
  规则除群消息外还能挂在「有人入群 / 有人退群 / 闹钟到点」上。
  `call_tool` 改走平台工具（AI 自己的身份），`run_script` 在它自己的沙箱里跑
  （脚本把要说的话以 JSON 打印出来，由平台代发）。
- **决策技能的命中结果会告知本体**：`notify=true` 时规则照跑、本体照被唤醒，
  并且看得到"技能已经做了什么"，不必重复劳动；入群/退群这类没有唤醒链路的情景，
  结果留在账本里等它下一轮看到。
- **情景名写错会被当场拦下**：规则只能挂在平台预置的情景上，写错时直接返回可选清单，
  不再出现"存下来了却永远不触发"。

### ⚡ 优化
- **全量群消息下的开销**：一条消息要过群里所有 AI 的决策规则，规则改为按消息批量取一次，
  不再每个 AI 各查一次库。
- **世界感知群消息不再等窗口**：世界程序收到群消息的第一条立即触发，随后一段时间内的
  消息合并成一条再触发一次——世界对第一条消息的延迟不再等于窗口长度。
