...
- **站内弹窗认 @ 了，正文也变成人话**：常驻通知的 push 载荷新增两样——`preview`（@令牌换成
  人名、Markdown 排版符号去掉、链接只留文字、纯附件给成 `[图片]`/`[3个文件]`；只在正文真有
  令牌时才查一次库，没人开着浮窗时连这次整理都不做）与逐收件人算出的两个标记：`mentioned_me`
  （点名到你个人）与 `mentions_all`（@all）。前端据此把被点名的弹窗换成 @ 图标 + 红色强调、
  不自动收起，标题分别挂「有人 @ 你」/「@ 全体成员」（其余仍 6 秒后收）。
  **免打扰语义**：免打扰挡的是"常规消息"而不是"别叫我"——点名到你个人照弹（与平台既有的
  「免打扰挡闲聊但 @ 穿透」一致），@all 不破例（喊所有人不该打扰关掉免打扰的人）——
  @ai 是叫 AI，两种都不算到人头上；没开免打扰的群任何消息照旧弹。
  同一个决定的三处执行点一次改齐：浮窗（弹）、标题未读计数（免打扰群里被点名到个人记 1，
  不把几十条闲聊一起算进来）、侧栏「@你」标记（不再被免打扰压掉）——只改一处会让用户
  看到自相矛盾的行为，比三处都不破例更糟。
- **`plainify_markdown` 会把标识符里的下划线吃掉**（**改之前就有的 bug**）：斜体规则无脑配对
  `_..._`，AI 写的 `snake_case_name`/`file_read` 会变成 `snakecasename`/`fileread`——
  QQ 纯文本降级一直如此，站内浮窗接上这条链路后同样受影响。按 CommonMark 的「词内下划线不算
  强调」加上边界判定；顺带修掉纯图片消息（`![](url)`）被洗成空白——现在退回 `[图片]` 占位。

### 🐞 修复
- **浮窗的三个小坑**：`describe` 的 switch 没有穷尽检查（新增 kind 忘了加分支只会在运行时炸），
  补 `never` 兜底；自动收起计时器把 `onDismiss` 放进了依赖，而父组件（Layout）每次渲染都给
  新函数——6 秒计时器会被反复重置、浮窗永远不消失，改成 ref 持有回调、依赖只留真正相关的三项；
  无跳转目标的卡片光标不是 pointer（点得动却看着点不动），现在一律 pointer，只有有目标时才
  加悬停边框。顺带把「个人点名优先于 @all」的优先级、"@全体成员"也走同一套强调色写进注释，
  `joinParts` 的判空从 `!!p` 收紧到只认字符串（原来会把将来可能传入的 0 静默丢掉）。
...
- **QQ 通道的回复会偶尔发不出去，停通道时还会打断正在发的那条**：出站回复用
  `asyncio.create_task` 起了就不管——事件循环只持弱引用，任务可能在途中被 GC 回收，表现是
  「偶尔有条回复没发出去，日志里什么都没有」；而 `stop()` 紧跟着 `aclose()` 掉 HTTP 客户端，
  在飞的那条请求正好打在已关闭的客户端上。两条通道（官方 QQ / NapCat）的后台回复改为统一交给
  `services/plugin/tasks.py` 托管：留强引用、异常写一条带实例与用途的日志、停通道前先 drain。
- **通道里两张只涨不落的表**：配对码通知表（每个陌生 QQ 一条，实际只有 60 秒有用）与私信路由表
  （每个「QQ 用户 ↔ 这个 AI」一条）都没有上限。前者过期即清，后者满 500 条丢最早的一条并记日志
  （那个人下次来消息会重新入表）。
- **@ 映射每次把整条通道的身份全表拉出来**：出站把 `<@!平台id>` 翻成通道侧的 @ 时，原先按
  (kind, owner_scope) 取全部配对用户再在 Python 里筛——一次回复只 @ 1~3 人，却付 O(通道历史) 的代价。
  改成只查被点名的那几个 id，锚点邮箱的反解与正写收在同一个模块里；仍只认走过这个实例的身份。
- **手打 @机器人名 唤醒时会补一个多余的令牌**：协议端没有 at 段、正文里自己写着名字时，原先照样补
  `<@!id>`，AI 会看到自己被点了两次名。现在名字已经命中就不补（名字命中同样唤醒，只是不再重复点名）。
- **聊天窗的虚拟列表 memo 一直是失效的**：`isOwnMessage` 每渲染新建一个函数、又挂在
  `messageElements` 的依赖里，于是打字、滚动、任何 state 变化都会重跑整段 slice/map 与引用查询——
  memo 没白写，但只剩「DOM 少渲染」这点收益。改成 `useCallback([userId])` 的稳定引用，顺带修掉
  `canRevoke` 闭包捕获首个渲染那份 `isOwnMessage` 的问题（换账号后撤回判定还按旧 user.id）。
- **新消息的 50ms 滚动与打字提示的 1.5s 回退没有清理**：切对话后这两个 timer 照样执行——
  前者会把刚进去的新对话也拽到底部（本该停在首个未读处），后者会清掉新对话的思考状态。
  收进一个 Set，切对话与卸载时统一 clear。
- **引用回复的查找从「窗口内每条都从头 find」改成按 id 直取的 Map**（每次滚动都要重算，
  等于 O(窗口 × 全量)）；同时清掉一处只写不读的状态（`peerType`，连带那次只为它发的
  `?summary=true` 请求）、四个没用到的图标导入，并把上下哨兵的 hook 提到模块级。
  另外把消息高度估算的已知误差来源写进注释（只估不量：图片与长中文都偏，靠 overscan 消化）。
