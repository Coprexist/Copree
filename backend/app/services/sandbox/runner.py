...
# ⚠️ 实测：24MB 下 python -I 连标准库都 import 不动（RLIMIT_AS 是虚拟内存口径，解释器约需 ≥32MB）。
# 故多解释器（默认形态）上限 64MB，单解释器共享场景下限 32MB（policy 硬下限）。
DEFAULT_RUNTIME_MEMORY_MB = 128  # 有人在线内存配额（runtime_memory_mb 可覆盖）
...
# 全局沙箱并发上限：排队的是「一次执行任务」而非人/群，世界与 AI 共用一个池
# （分开只会把上限拆小）。在线不占位，跑完释放；SANDBOX_MAX_CONCURRENT 可配（默认 4）。
...
# ── 子进程入口：先隔离，再跑目标 ──
# 目标是调用方给的脚本/入口或平台 harness；隔离只在这里施加一次，harness 内不重复。
_RUNNER_TEMPLATE = r'''
import os, runpy, sys
