...
| 内联 @ | **只在 Markdown 下可用** | 正文里的 `<@!openid>` 渲染成**可点的 @对方**；平台据此把平台的 @ 翻成真 @。**纯文本消息渲染不了**：2026-09-26 真机自测，带 `!` 与不带 `!` 两种写法都原样显示成尖括号 → 配置成纯文本时平台退成 `@名字` |
...
| 接收全量群消息 | 群里可开 | 群主在 **QQ 群设置 → 群机器人 → 该机器人 → 机器人可获取的群聊消息范围 → 获取群内全部消息**（不在开放平台；每个群单独设）。开了之后群里每条消息都进平台，**只有点名到机器人的那条带唤醒令牌**；@别人时腾讯写在正文里的原始提及（`<@openid>`）会被换成平台的 `<@!id>` |
| 切换全量开关 | 会告诉 AI | 这个开关腾讯**不发通知**，平台从事件类型看出来（`GROUP_MESSAGE_CREATE` = 在喂全量）；切了之后往 AI 的账本补一条「通道变更」通知，它每轮读到的规则说明也跟着换 |
| 被回复时自带 @ | 分场景 | 只有回复**「@ 事件」送来的消息**时腾讯自己会补一个 @对方（我们就不重复写）；全量事件送来的消息——哪怕正文 @ 了机器人——它不补，这时 @ 由我们自己发（**2026-09-26 真机**：08:28 的 @ 事件回复出现了 @，11:00 的全量事件回复没有） |
