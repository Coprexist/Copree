...
1. **出站翻译**：把 `<@!id>` 换成通道侧的写法（QQ 官方 `<@!openid>`、NapCat `[CQ:at,qq=…]`）。
   ...
   查库放在后台发送任务里，别拖慢发消息。
   两条平台差异（QQ 官方实测）：**消息类型决定内联 @ 认不认**（Markdown 认、纯文本不认，后者退名字）；
   **只有 @ 事件送来的消息，腾讯才自带 @对方**（那种情况要摘掉正文开头对同一人的 @，免得两个 @）——
   细节见 [@ 提及统一用 id](./mention_ids.md)。
...
有些平台支持"群里每条消息都推送"（QQ 官方是 `GROUP_MESSAGE_CREATE`，需群主在群设置里开启）。接它时注意四条：

1. **每条都进 Copree，只有点名到机器人的那条加唤醒令牌**：全量事件里官方给 `mentions`（带 `bot` 标记），
   优先用它判"这条点到机器人没有"，拿不到再退回按名字判；
   注意正文里的原始提及是 `<@openid>`（没有 `!`）——按 `mentions` 里的 origin 摘掉，
   机器人自己那条只摘不补，别的成员摘掉后补 `<@!id>`；
2. **按消息 id 去重**：同一条消息在 @模式与全量模式会各来一次，共用同一张去重表；
3. **更完整的一方胜出**：后到的事件正文更长时，把库里的正文补上
   （@模式的正文可能在 @其他成员处被截断；账本里已写下的条目不改——append-only）；
4. **开关变了要告诉 AI**：这个开关没有通知，只能从事件类型判（`GROUP_MESSAGE_CREATE` = 在喂全量、
   `GROUP_AT_MESSAGE_CREATE` = 只喂点名）。两件事都要做：① 平台注入的通道说明按当前模式选文案
   （观测不到就两句都讲，不猜）；② 模式**翻转**时往 AI 的账本补一条 `notice`，幂等以账本为准
   （重启不丢、不重复投）。细节见 [@ 提及统一用 id](./mention_ids.md)。
