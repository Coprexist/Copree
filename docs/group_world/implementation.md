...
- **这么做的原因**：固定配额对“群里人多”和“没人”的世界不公平——人越多世界代码被调用越频，配额应随之放大；写操作（发群消息/管理）比读更敏感，独立更严的配额。
...
- **这么做的原因**（各用各的解释器 + 排队）：共享解释器（多群一进程）隔离是硬伤（一个世界死循环卡死全部、全局变量互踩），放弃；排队的是**短任务**不是人——在线不占位，只有执行中的几秒占槽位。
...
- **这么做的原因**（保留自动）：迁移本身正确就不必关，其他人改了能马上用；防护放在生成端。
...
- **移动手感**：**按一下走一格**（经典 RPG 滑步，stepDuration 0.16s），按住不连续跑；触屏点一下走一格。
