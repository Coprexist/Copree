...
> **版本**: v0.4.12 | **更新**: 2026-09-26 | **阶段**: 正式版（功能完整，持续迭代）
>
> **v0.4.12 里程碑**：AI 自己的代码沙箱——把 AI 的文件空间本身锁成沙箱（只能读写自己那份、
> 不联网、不起子进程），新工具 `run_script`；世界代码与 AI 脚本共用同一套执行层（隔离/配额/
> 超时只剩一处实现）。决策技能面向全平台——不再要求绑定群视界，情景扩到群消息 / 入群 / 退群 /
> 定时，`call_tool` 用 AI 自己的身份、`run_script` 在它的沙箱里跑，命中结果会告知本体；
> 全量群消息下规则按消息批量取。世界感知群消息改为「首条立即 + 合并窗口」。
>
> **v0.4.11 里程碑**：记忆的适用范围与生命周期——焦段（会话轴 / 语义轴，AI 自选、可改名、可合并）、
> 设定权值与时间权值两档、低权值日常流水每日清理、记忆写入带类型与锚点、注入排在对话历史之前。
>
...
mindmap
  root((Copree))
    ...
    关键能力
      ...
      向量维度可配置 + 前端图形化
      记忆检索合并（关键词+向量）
      焦段：会话轴 + 语义轴（AI 自选、可合并）
      记忆类型与权值 · 每日整理
      统一上下文 v0.2.1
      状态栈跨任务追踪 v0.2.1
      群级DND增强 · @all穿透
      62个Agent工具 + 34个世界工具 · 7个技能段
      ...
      通道自测 · 可配正文格式与引用回复 · 全量群消息 · 推送模式变化通知 AI
...
#### 🧭 v0.4.11 焦段与记忆权值（记在哪、活多久）

v0.3.6 让向量检索重新可用，但记忆仍缺两样东西：**该在哪里被想起**、**该活多久**。v0.4.11 补上。

- **两条轴（焦段）**：会话焦段（一组同类对话，如「学生群」）与语义焦段（一段话题，如「化学教学」）
  平行正交，各自可多选、可空，由 AI 自己选、建、改名、合并（合并前强制列出两侧挂着的记忆）。
  平台预置「所有聊天」表示全局；归属随状态摘要每轮复述，AI 不必重新判断自己属于哪里。
- **空集不是全局**：一条记忆不锚任何焦段时，只在本会话与当前语义焦段可见，工具会明确提醒——
  没有隐式全局，要跨会话共享必须显式锚定。
- **两个权值**：设定权值（1-5，写入时给定，**永不随时间改动**）+ 时间权值（读取时现算，不落库）。
  有效权重 =（设定权值 / 5）× 1 / (1 + 主观时长 / 30 天)，主观时长取「真实静默天数」与
  「调用数折算天数」中更快的一个——聊得越多，主观上过得越久，旧事该退场。
- **两档处置**：最低档（日常流水）退场即物理删除、腾出空间；2 以上只是不再自动注入，记录保留、
  显式检索仍可取回。由每日后台整理执行（`services/memory/tidy_service.py`）。
- **写入即带身份**：`store_memory` / `manage_records` 支持类型（人物 / 关系 / 约定 / 经历 / 偏好 / 流水）、
  权值与焦段锚点；标题 30 字、内容 200 字以内，超了只提醒、不裁剪 AI 写的内容。
- **注入位置**：记忆排在对话历史之前——先想起这个人，再读他说的话。

> 设计见 [焦段与记忆适用范围](../memory_system/design/focus_and_memory_reach.md)；
> 端到端观测用 `scripts/memory_probe.py`（存一条记忆，看它能不能被想起来）。
...
### 3.5 🔧 62 个工具 · 7 个技能段 · 状态白名单

AI 的工具按**技能段**组织，不同状态下动态过滤可见工具（`ToolPlugin.__init_subclass__` 自动发现）：

| 技能段 | 工具数 | 代表工具 | 适用状态 |
|--------|--------|---------|----------|
| 群聊社交 | 23 | send_message, send_dm, send_file, switch_state, set_dnd, cancel_dnd, silence_member, enter_group, list_available_skills, web_search, web_fetch, roll_dice | active |
...
| 自我配置 | 5 | update_self_config, toggle_thinking, set_status, manage_skills | active/dnd/inactive |
| 自我管理 | 15 | end_turn, push_state, pop_state, close_state, list_states, set_alarm, update_alarm, cancel_alarm, list_alarms, compress_context, manage_workspace, check_workspace, clear_current_task, tool_help | 全状态 |
| 网络工具 | 2 | weather（Open-Meteo）, bilibili_summary（B站视频总结） | active |

| AI 状态 | 可用工具数 | 说明 |
|----------|-----------|------|
| active | 62 | 全部工具 |
| dnd | 46 | 排除社交主动工具，仍可管理自我 |
| inactive | 20 | 含状态栈全套工具（@提及可唤醒为 active） |
| blocked | 0 | 完全封禁 |

> ⚠️ 未知/遗留状态（如旧版 `offline`）由 `get_allowed_tools()` 兜底为 inactive 工具集（20 个），避免 AI 被触发后无工具可用导致死循环；blocked 保持 0 工具不受影响。
...
**2026-09-26 补充**：静音分三档，别混——**免打扰**（普通消息不唤醒，@/@all/群公告/特别关心**都穿透**）、
**屏蔽**（**连 @ 也不穿透**，AI 工具限 ≤30 分钟）、**按人静音**（只对**某个人**：连他的 @ 也不理，
可按分钟或"他再说 N 条"，两个都不给就是永久）。前两条管"整个群"，第三条管"某个人"。
...
| 群消息钩子 | 群消息 → 世界程序 `handle(event)` 异步感知（首条立即触发 + 合并窗口可配；`source=world` 防自触发死循环） |
...
| 世界 AI 下载 | web_download 工具：下载网页资源到世界文件夹（两阶段确认 + SSRF + 扩展名白名单 + 32MB） |
...
- 全量群消息模式：群里开启「接收所有消息」后每条消息都进平台（镜像完整对话），只有点名到机器人的那条带唤醒令牌；
  正文里的原始提及会换成平台的 `<@!id>`，"按消息 id 去重"是切换开关期间的兜底
- 推送模式会观测也会通知：全量开关腾讯不发通知，平台从事件类型看出来（`GROUP_MESSAGE_CREATE` = 在喂全量）；
  通道说明按当前模式换文案，**模式翻转时给 AI 的账本补一条「通道变更」通知**（幂等以账本为准，重启不丢）
...
| **工具系统** | 25 | 🟢 稳定 | **62 个 Agent 工具 + 34 个世界工具**（独立注册表）、7 个技能段、定义即自注册；缺 label/summary 契约直接抛错，另有 `test_world_tool_summaries.py` 兜底 |
| **状态栈** | 2 | 🟢 稳定 | v0.2.1: push/pop/close/list 状态帧 + end_turn 持久化 + workspace 联动 |
| **记忆系统** | 5 | 🟡 完善中 | 向量 + 结构记忆双轨、批量缓冲、类型与设定权值、焦段适用范围、低权值每日清理（v0.4.11 起；锚点尚未参与召回） |
...
| [@ 提及统一用 id](../dev/mention_ids.md) | 开发者 | 令牌写法、入口归一、识别兼容、通道出站翻译、群推送模式观测与通知、展示层与测试 |
