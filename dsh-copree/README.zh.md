# dsh-copree — Copree 接入 DeepSeek Harness Web 的插件

[English](README.md) | 中文

热插拔的 DeepSeek Harness（DSH）Web 插件：把 [Copree](https://github.com/Coprexist/Copree)
——开源的 **AI 群聊与可编程世界框架**——装成 DSH 原生的侧边栏面板、沉浸式覆盖层，以及
**一个群视界 = 一个 DSH 工作区**。它通过 `cordis.patch.yml` 与 profile 机制挂载，不改 DSH 源码。

- 两个半侧：**Host 半**在 DSH Web 服务上注册同源网关，**浏览器半**把 Copree 界面注入 DSH 的槽位。
  关掉页面既不会停掉世界镜像，也不会停掉面板自己的流量。
- **反向桥接默认关**（Copree 管理端驱动真实 DSH 会话这条链路）：必须由人在 **DSH 里**打开
  （设置 → Copree → 同意接入）。在那之前 Host 不发心跳、不注册任何桥接路由——Copree 连
  「检测到有台 DSH 在运行」都做不到。
- 群视界文件是**镜像**：agent 用 DSH 原生工具（`read`/`write`/`edit`/`bash`）编辑，再用
  `world_push` / `world_pull` 同步；冲突如实报告，绝不静默覆盖。
